            ...
            <Div>
                { requestsList.slice(0,limit).map(request=>{
                   const {userId, username} = request;
                    return <RequestType key={userId} id={userId}>
                            <Typo14DarkBlueGreyHKGroteskRegular>{username.toUpperCase()}</Typo14DarkBlueGreyHKGroteskRegular>
                            <RequestApprovals> 
                                <Icons><IoIosCloseCircleOutline color={cool_grey}/> </Icons>
                                <Icons><IoIosCheckmarkCircle color={ bright_blue}/></Icons>    
                            </RequestApprovals>
                        </RequestType>
                })}
